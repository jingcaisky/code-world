...
**multi_agent** - FastAPI application generated with [Full-Stack AI Agent Template](https://github.com/vstorm-co/full-stack-ai-agent-template).

**Stack:** FastAPI + Pydantic v2, PostgreSQL (async via asyncpg)
, JWT + API Key auth, Redis, LangGraph, RAG (pgvector), Celery, Next.js 15 (i18n)
...
# RAG
uv run multi_agent rag-collections
uv run multi_agent rag-ingest /path/to/file.pdf --collection docs
uv run multi_agent rag-search "query" --collection docs

# Sync Sources
uv run multi_agent cmd rag-sources
uv run multi_agent cmd rag-source-add
uv run multi_agent cmd rag-source-sync
```
