...
dev:
	...
	docker compose -f docker-compose.dev.yml exec -T app multi_agent db upgrade
	@echo ""
...
seed:
	...
	@if docker compose -f docker-compose.dev.yml exec -T app \
		multi_agent user list 2>/dev/null \
...
stage:
	...
	docker compose -f docker-compose.yml exec -T app multi_agent db upgrade
...
prod:
	...
	@sleep 5
	docker compose --env-file backend/.env -f docker-compose.prod.yml exec -T app multi_agent db upgrade
...
db-init: docker-db
	...
	@sleep 8
	cd backend && uv run multi_agent db migrate -m "initial" || true
	cd backend && uv run multi_agent db upgrade
	@echo ""
	@echo "✅ Database initialized!"

db-migrate:
	@read -p "Migration message: " msg; \
	uv run --directory backend multi_agent db migrate -m "$$msg"

db-upgrade:
	uv run --directory backend multi_agent db upgrade

db-downgrade:
	uv run --directory backend multi_agent db downgrade

db-current:
	uv run --directory backend multi_agent db current

db-history:
	uv run --directory backend multi_agent db history

# === Server ===
run:
	uv run --directory backend multi_agent server run --reload

run-prod:
	uv run --directory backend multi_agent server run --host 0.0.0.0 --port 8000

routes:
	uv run --directory backend multi_agent server routes

# === Users ===
create-admin:
	@echo "Creating admin user..."
	uv run --directory backend multi_agent user create-admin

user-create:
	uv run --directory backend multi_agent user create

user-list:
	uv run --directory backend multi_agent user list

# === Celery ===
celery-worker:
	uv run --directory backend multi_agent celery worker

celery-beat:
	uv run --directory backend multi_agent celery beat

celery-flower:
	uv run --directory backend multi_agent celery flower
	@echo ""
...
docker-db:
	...
	@echo "   Connection: postgresql://postgres:postgres@localhost:5432/multi_agent"
...
help:
	@echo ""
	@echo "multi_agent - Available Commands"
	@echo "======================================"
	@echo ""
	...
	@echo ""
	@echo "RAG:"
	@echo "  uv run multi_agent rag-ingest <path> -c <collection>  Ingest files"
	@echo "  uv run multi_agent rag-search <query> -c <collection>  Search"
	@echo "  uv run multi_agent rag-collections                     List collections"
	@echo "  uv run multi_agent rag-sources                         List sync sources"
	@echo "  uv run multi_agent rag-source-add                      Add sync source"
	@echo "  uv run multi_agent rag-source-sync <id>                Trigger sync"
	@echo ""
	@echo "Celery:"
