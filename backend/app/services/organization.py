...
class OrganizationService:
    ...
    async def get_by_id(self, org_id: UUID) -> Organization:
        ...
        if not org:
            raise NotFoundError(message="组织未找到", details={"org_id": str(org_id)})
        return org
    ...
    async def update(
        ...
        if membership.role not in (OrgRole.OWNER.value, OrgRole.ADMIN.value):
            raise AuthorizationError(message="仅所有者或管理员可以更新组织")
    ...
    async def delete(self, org_id: UUID, requester_id: UUID) -> None:
        ...
        if org.is_personal:
            raise BadRequestError(message="个人组织无法删除")
        if membership.role != OrgRole.OWNER.value:
            raise AuthorizationError(message="仅所有者可以删除组织")
    ...
    async def upload_avatar(
        ...
        if content_type not in self._ALLOWED_AVATAR_TYPES:
            raise BadRequestError(message="仅支持 JPEG、PNG、WebP 和 GIF 格式")
        if len(file_data) > 2 * 1024 * 1024:
            raise BadRequestError(message="头像图片过大，最大 2MB")
