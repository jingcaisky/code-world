...
class UserService:
    ...
    async def authenticate(self, email: str, password: str) -> User:
        ...
        if (
            ...
            raise AuthenticationError(message="邮箱或密码错误")
        if not user.is_active:
            raise AuthenticationError(message="用户账户已禁用")
        return user
    ...
    async def update_avatar(
        ...
        if content_type not in ALLOWED_AVATAR_TYPES:
            raise ValueError("仅支持 JPEG、PNG、WebP 和 GIF 格式的图片")
        if len(file_data) > 2 * 1024 * 1024:
            raise ValueError("头像图片过大，最大 2MB")
    ...
    async def confirm_password_reset(self, token: str, new_password: str) -> User:
        ...
        try:
            ...
            raise AuthenticationError(message="重置链接无效或已过期") from exc

        user = await self.get_by_id(user_id)
        if not user.is_active:
            raise AuthenticationError(message="账户已禁用")
    ...
    async def consume_magic_link_token(self, token: str) -> User:
        ...
        if payload is None or "sub" not in payload:
            raise AuthenticationError(message="魔法链接无效或已过期")
        try:
            user_id = UUID(str(payload["sub"]))
        except (TypeError, ValueError) as exc:
            raise AuthenticationError(message="魔法链接无效或已过期") from exc
