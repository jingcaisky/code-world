...
```bash
cd backend
uv run multi_agent <group> <command> [options]
```

### Server Commands

```bash
uv run multi_agent server run              # Start dev server
uv run multi_agent server run --reload     # With hot reload
uv run multi_agent server run --port 9000  # Custom port
uv run multi_agent server routes           # Show all registered routes
```
...
```bash
uv run multi_agent db init                  # Run all migrations
uv run multi_agent db migrate -m "message"  # Create new migration
uv run multi_agent db upgrade               # Apply pending migrations
uv run multi_agent db upgrade --revision e3f  # Upgrade to specific revision
uv run multi_agent db downgrade             # Rollback last migration
uv run multi_agent db downgrade --revision base  # Rollback to start
uv run multi_agent db current               # Show current revision
uv run multi_agent db history               # Show migration history
```
...
uv run multi_agent user create

# Create user non-interactively
uv run multi_agent user create --email user@example.com --password secret

# Create user with specific role
uv run multi_agent user create --email admin@example.com --password secret --role admin

# Create user with superuser flag
uv run multi_agent user create --email admin@example.com --password secret --superuser

# Create admin (shortcut)
uv run multi_agent user create-admin --email admin@example.com --password secret

# Change user role
uv run multi_agent user set-role user@example.com --role admin

# List all users
uv run multi_agent user list
```

### Celery Commands

```bash
uv run multi_agent celery worker                    # Start worker
uv run multi_agent celery worker --loglevel debug   # Debug logging
uv run multi_agent celery worker --concurrency 8    # 8 worker processes
uv run multi_agent celery beat                      # Start scheduler
uv run multi_agent celery flower                    # Start Flower UI
uv run multi_agent celery flower --port 5556        # Custom Flower port
```
...
```bash
uv run multi_agent cmd <command-name> [options]
```
...
uv run multi_agent cmd rag-ingest ./docs/guide.pdf

# Ingest a directory
uv run multi_agent cmd rag-ingest ./docs/

# Ingest recursively into a specific collection
uv run multi_agent cmd rag-ingest ./docs/ --collection knowledge --recursive

# Ingest with sync mode
uv run multi_agent cmd rag-ingest ./docs/ --sync-mode new_only
uv run multi_agent cmd rag-ingest ./docs/ --sync-mode update_only

# Skip replacing existing documents
uv run multi_agent cmd rag-ingest ./docs/ --no-replace
```
...
uv run multi_agent cmd rag-search "what is fastapi"

# Search a specific collection
uv run multi_agent cmd rag-search "deployment guide" --collection docs

# Get more results
uv run multi_agent cmd rag-search "deployment" --top-k 10
```
...
uv run multi_agent cmd rag-collections

# Show overall RAG system statistics
uv run multi_agent cmd rag-stats

# Drop a collection (with confirmation)
uv run multi_agent cmd rag-drop my_collection

# Drop without confirmation
uv run multi_agent cmd rag-drop my_collection --yes
```
...
uv run multi_agent cmd rag-source-add \
    --name "My Drive" \
    --type gdrive \
    --collection docs \
...
uv run multi_agent cmd rag-source-remove <source-id>
uv run multi_agent cmd rag-source-remove <source-id> --yes  # Skip confirmation

# Trigger sync for a specific source
uv run multi_agent cmd rag-source-sync <source-id>

# Trigger sync for all active sources
uv run multi_agent cmd rag-source-sync --all
```
...
```bash
uv run multi_agent cmd my-command --name test
```
