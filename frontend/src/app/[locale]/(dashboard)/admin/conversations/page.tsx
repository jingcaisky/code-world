  ...
  const toggle = async () => {
    ...
    try {
      ...
      onToggle(conv.id, next);
        toast.success(next ? "已加入演示" : "已移出演示");
    } catch {
      toast.error("网络错误，无法更新演示状态");
    } finally {
      setBusy(false);
    }
  };
  return (
    <span className="inline-flex items-center gap-1.5">
      <button
        ...
        disabled={busy}
        onClick={toggle}
        title={conv.is_demo ? "从演示中移除" : "加入演示"}
        className={cn(
...
export default function AdminConversationsPage() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="text-muted-foreground text-xs">共 {conversationsTotal} 条</div>
