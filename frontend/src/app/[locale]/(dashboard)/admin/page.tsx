    ...
    <div className="space-y-6">
      ...
      {statsQuery.isLoading ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <StatCard label="用户总数" value={(stats?.total_users ?? 0).toLocaleString()} icon={Users} />
          <StatCard
            label="24 小时活跃"
      ...
      <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        <QuickLink href={ROUTES.ADMIN_USERS} icon={Users} title="管理用户" description="搜索、停用、冒充登录" />
        <QuickLink
          href={ROUTES.ADMIN_CONVERSATIONS}
          icon={MessageSquare}
          title="浏览聊天"
          description="所有用户的全部对话"
        />
        <QuickLink
          href={ROUTES.ADMIN_STRIPE_EVENTS}
          icon={CreditCard}
          title="Stripe 事件"
          description="重放 Webhook，调试计费"
        />
        <QuickLink href={ROUTES.ADMIN_SYSTEM} icon={Activity} title="系统健康" description="各服务状态与在线率" />
        <QuickLink href={ROUTES.ADMIN_RATINGS} icon={Star} title="回复评分" description="来自用户的质量反馈信号" />
      </section>

      <section className="border-border bg-card rounded-xl border">
        <div className="border-border border-b px-5 py-4">
          <h2 className="text-foreground text-sm font-semibold">最近活动</h2>
