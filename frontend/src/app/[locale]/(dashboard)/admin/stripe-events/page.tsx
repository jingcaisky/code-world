...
export default function StripeEventsPage() {
  ...
  const handleReplay = async (evt: StripeEvent) => {
    if (usingStub) {
      toast.info("演示模式 - 需要后端接线（POST /admin/stripe-events/{id}/replay）");
      return;
    }
    setReplaying(evt.id);
    try {
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="border-border bg-muted flex items-start gap-3 rounded-xl border p-3">
          ...
          <div className="min-w-0 flex-1 text-xs">
            ...
            <p className="text-muted-foreground mt-0.5">
              需要后端接线。预期接口：<code className="font-mono">GET /admin/stripe-events</code>，{" "}
      ...
      <div className="text-muted-foreground text-xs">共 {total} 条</div>
      ...
      {total > 0 && (
        <div className="border-border bg-card flex items-center justify-between rounded-xl border px-4 py-3">
          <span className="text-muted-foreground text-sm">
            {page * pageSize + 1}–{Math.min(total, (page + 1) * pageSize)} / 共 {total}
          </span>
...
function StatusBadge({ status, attempts }: { status: StripeEvent["status"]; attempts: number }) {
  ...
  const label = { processed: "已处理", failed: "失败", pending: "待处理" }[status];
  return (
    <span
      className={cn(
        ...
        {event && (
          <>
            ...
            <dl className="grid gap-3 text-xs sm:grid-cols-2">
              <KV label="模式" value={event.livemode ? "正式" : "测试"} />
              <KV label="状态" value={event.status} />
              <KV label="尝试次数" value={String(event.attempts)} />
              <KV label="创建时间" value={formatDateTime(event.created_at)} />
              {event.customer_email && <KV label="客户" value={event.customer_email} />}
              {typeof event.amount_cents === "number" && (
                <KV label="金额" value={formatAmount(event.amount_cents, event.currency)} />
              )}
              {event.last_error && (
                <KV label="最后错误" value={event.last_error} accent="danger" />
              )}
            </dl>
