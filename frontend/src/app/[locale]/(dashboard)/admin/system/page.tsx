...
function buildServices(resp: BackendHealthResp | null): ServiceHealth[] {
  ...
  return [
    ...
    },
    {
      key: "llm",
      name: "大模型提供方",
      description: resp?.llm?.provider ? `提供方：${resp.llm.provider}` : "默认模型接口",
      icon: Cpu,
...
export default function SystemHealthPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(getErrorMessage(err, "获取健康状态失败"));
    } finally {
      setLoading(false);
    }
  };
      ...
      <section className="border-border bg-card rounded-xl border p-5">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-muted-foreground text-xs">总体状态</p>
          ...
          {lastChecked && (
            <span className="text-muted-foreground text-xs">
              已检查 {lastChecked.toLocaleTimeString()}
            </span>
          )}
        </div>
      </section>
      ...
      {loading && !resp ? (
        ...
        <section className="border-border bg-card rounded-xl border">
          <div className="border-border border-b px-5 py-4">
            <h2 className="text-foreground text-sm font-semibold">服务</h2>
            <p className="text-muted-foreground text-xs">
              各个后端服务的实时就绪状态。每 30 秒自动刷新。
            </p>
          </div>
